...
class SCRFD:
    def __init__(self, model_file=None):
        ...
        self.batched = False
        self.session = onnx_model_inference(self.model_file)
        self.center_cache = {}
        self.nms_thresh = 0.4
        self._init_vars()
    ...
    def get(self, img, threshold=0.5, input_size=(640, 640)):
        bboxes, kpss = self.detect(
            img, thresh=threshold, input_size=input_size)
        # return bboxes,kpss
        if bboxes.shape[0] == 0:
            return []
        ret = []
